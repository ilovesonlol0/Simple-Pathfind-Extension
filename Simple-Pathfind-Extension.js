class SimplePathFind {
  ...
  generateCollisionGrid() {
    ...
    const sprite = targets.find(t => t.sprite?.name === this.collisionSprite);

    if (!sprite || !sprite.drawable) {
      console.warn("[SimplePathFind] Collision sprite not found or has no drawable.");
      return;
    }

    const skin = sprite.drawable.skin;
    if (!skin || !skin._canvas) {
      console.warn("[SimplePathFind] No canvas available for collision skin.");
      return;
    }
    ...
    ctx.drawImage(skin._canvas, 0, 0);
  ...
  getPathLength(args) {
    const id = args.ID;
    return this.paths[id]?.length || 0;
  }

  getWaypoint(args) {
    const id = args.ID;
    const index = Math.floor(args.INDEX);
    return this.paths[id]?.[index]?.x?.toString() ?? "";
  }

  getWaypointY(args) {
    const id = args.ID;
    const index = Math.floor(args.INDEX);
    return this.paths[id]?.[index]?.y?.toString() ?? "";
  }
